...
def main():
    ...
    if datos:
        ...
        try:
            ...
            subprocess.run(["git", "add", "Briefing_Lunes_ADN.html", "procesador_emails_nn.py"], cwd=carpeta_scripts, check=True)
            if os.path.exists(ruta_json):
                subprocess.run(["git", "add", "../radar_db.json"], cwd=carpeta_scripts, check=True)
