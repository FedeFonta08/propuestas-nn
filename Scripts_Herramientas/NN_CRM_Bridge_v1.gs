...
function doGet(e) {
  // Permite OPTIONS preflight y comprobación de estado
  if (e.parameter.test === '1') {
    return ContentService
      .createTextOutput(JSON.stringify({ ok: true, msg: 'NN CRM Bridge activo', ts: new Date().toISOString() }))
      .setMimeType(ContentService.MimeType.JSON);
  }
  return ContentService
    .createTextOutput(JSON.stringify({ ok: true, msg: 'NN CRM Bridge v1 · GET OK' }))
    .setMimeType(ContentService.MimeType.JSON);
}
